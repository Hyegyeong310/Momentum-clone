const texts: { text: string; who: string }[] = [
  {
...
const saying: HTMLElement = document.querySelector('.js-saying');
const text: HTMLElement = saying.querySelector('.js-saying-title');
const who: HTMLElement = saying.querySelector('.js-who');

function paintText(textIndex: number): void {
  text.innerText = texts[textIndex].text;
  who.innerText = texts[textIndex].who;
}

function getRandom(): number {
  const number: number = Math.floor(Math.random() * texts.length);
  return number;
}

function sayingInit(): void {
